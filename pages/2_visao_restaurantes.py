...
def avg_std_time_delivery(df1, festival, op):
    """ Essa função calcula o tempo médio e o desvio padrão do tempo de enterga.
        Parâmetros:
        Input:
            - df: dataframe com os dados necessários para o calculo
            - festival: 'yes' or 'no'
            - op: tipo de operação pretendida (responder com 'avg' ou 'std')
                avg calcula o tempo médio, std calcula o desvio padrão
        Output:
...
with tab1:
    with st.container():
        ...
        with col3: 
            ...
            col3.metric('Tempo entrega médio Festivais', df_aux )  

        with col4:
            df_aux = avg_std_time_delivery(df1, 'Yes', 'std_time')
            col4.metric('DP entrega Festivais', df_aux )
            
        with col5:
            df_aux = avg_std_time_delivery(df1, 'No', 'avg_time') #incluído mais um parâmetro (festival)
            col5.metric('Tempo entrega médio', df_aux )          
            
        with col6:
            df_aux = avg_std_time_delivery(df1, 'No', 'std_time')
            col6.metric('DP entregas', df_aux )
